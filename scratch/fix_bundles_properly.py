...
with open('server/synthetic_data.py', 'r', encoding='utf-8') as f:
    text = f.read()

# Make sure we don't add them twice
if "Blizzard Whiteout" not in text:
    # Find the end of EPISODE_BUNDLES which is exactly:
    target = '''        task3=Task3Scene(
    ...
    with open('server/synthetic_data.py', 'w', encoding='utf-8') as f:
        f.write(text)
    print("synthetic_data.py patched successfully.")
else:
    print("Bundles already exist in synthetic_data.py")
